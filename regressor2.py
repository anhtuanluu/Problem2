...
for fold, (train_dx, test_dx) in enumerate(splits):
    ...
    tq = tqdm(range(args.epochs + 1))
    num_train_optimization_steps = int(args.epochs * len(train_dataset)/args.batch_size)
    ...
    for epoch in tq:
        ...
        if avg_eval_loss <= best_score:
            torch.save(mymodel.state_dict(), f"/content/model_{fold}.bin")
