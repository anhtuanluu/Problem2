...
for fold, i in enumerate(splits):
    ...
    if fold != args.fold:
        continue
    train_dataset = torch.utils.data.TensorDataset(torch.tensor(data_npy[:2955],dtype=torch.long), torch.tensor(target_npy[:2955],dtype=torch.long))
    valid_dataset = torch.utils.data.TensorDataset(torch.tensor(data_npy[2955:],dtype=torch.long), torch.tensor(target_npy[2955:],dtype=torch.long))
    tq = tqdm(range(args.epochs + 1))
    ...
    for epoch in tq:
        ...
        for i,(x_batch, y_batch) in pbar:
            ...
            avg_eval_loss += loss.item() / len(valid_loader)
        ...
        best_th = 0
        score = f1_score(target_npy[2955:], val_preds)
